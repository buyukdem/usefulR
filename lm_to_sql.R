...
{
  if (is.na(names(regressor1_lm$xlevels[1]))) {
    
    sql1 <- paste0(x$coefficients[1]," + ")
    
    for (i in 2:length(x$coefficients)) {
      
      sql0 <- paste0("(",x$coefficients[i],")*",
                     "isnull(",names(x$coefficients[i]),",0)"," + ")
      
      sql1 <- paste0(sql1,sql0)
    }
    sql <- substr(sql1,1,nchar(sql1)-3)
